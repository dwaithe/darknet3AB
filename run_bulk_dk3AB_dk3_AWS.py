...
def run_experiment(exp_id,GPU_to_use):
	...
	for darknet_models, train_on_dataset, test_on_dataset_arr, number_of_reps, flip_on, num_of_classes in zip(dmia, traina, testa, rep_a, flip_a, classes):
		...
		if flip_on == "True":
			cfg_file='yolov3_dk3AB-classes-'+str(int(num_of_classes))+'-flip'
		elif flip_on == "False":
			cfg_file='yolov3_dk3AB-classes-'+str(int(num_of_classes))+'-no-flip'
		else:
		...
		for rep in range(0,number_of_reps):
			...
			#Training.
			if to_train == True:
				darknet_model_init = darknet_models['darknet3_model_init']
